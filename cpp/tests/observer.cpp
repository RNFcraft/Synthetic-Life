...
int main() {
    ...
    se::DialogueSnapshotChannel dialogue;dialogue.publish(1.25,se::DialogueRole::External,"dax");auto retained_dialogue=dialogue.latest();dialogue.publish(1.5,se::DialogueRole::Entity,"future-output");assert(retained_dialogue->revision==1&&retained_dialogue->lines.size()==1&&retained_dialogue->lines[0].text=="dax"&&retained_dialogue->lines[0].role==se::DialogueRole::External);for(int n=0;n<100;++n)dialogue.publish(2.+n,se::DialogueRole::External,"line-"+std::to_string(n));auto bounded_dialogue=dialogue.latest();assert(bounded_dialogue->lines.size()==se::kMaxDialogueLines&&bounded_dialogue->lines.front().sequence<bounded_dialogue->lines.back().sequence&&bounded_dialogue->lines.back().text=="line-99");
