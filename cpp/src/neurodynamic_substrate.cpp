...
std::uint32_t NeurodynamicSubstrate::add_micro_rho(std::uint32_t source,std::uint32_t target,double weight,double delay,MicroPolarity polarity,bool enabled) {
  if (source>=potential_.size() || target>=potential_.size() || !std::isfinite(weight) || !std::isfinite(delay) || weight<weight_min_ || weight>weight_max_ || delay<=0 || (polarity!=MicroPolarity::Excitatory && polarity!=MicroPolarity::Inhibitory)) throw std::invalid_argument("invalid micro rho");
...
void NeurodynamicSubstrate::restore(const NeuroSnapshot&s) {
  ...
  for(std::size_t i=0;i<s.source.size();++i) if(s.source[i]>=s.potential.size() || s.target[i]>=s.potential.size() || !std::isfinite(s.weight[i]) || s.weight[i]<s.weight_min || s.weight[i]>s.weight_max || !std::isfinite(s.delay[i]) || s.delay[i]<=0 || (s.polarity[i]!=(std::uint8_t)MicroPolarity::Excitatory && s.polarity[i]!=(std::uint8_t)MicroPolarity::Inhibitory) || s.plasticity_enabled[i]>1) throw std::invalid_argument("invalid snapshot rho");
