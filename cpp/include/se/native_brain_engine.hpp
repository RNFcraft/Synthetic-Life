...
struct MaterializedRelation {
  std::uint32_t source, target, support;
  std::uint8_t action;
  double probability, confidence, lift;
};
struct PlannerTransition {
  std::vector<std::vector<std::pair<std::uint32_t, double>>> predictions, effects;
};
struct EvidenceConfig {
  std::uint32_t minimum_support{3}, consolidated_support{20};
  double minimum_lift{1}, confidence_k{8}, consolidated_confidence{.68};
};
struct AssemblyCognitBridgeResult {
  std::uint64_t event_sequence{}, assembly_id{};
  std::uint32_t cognit_id{};
  double confidence{};
  std::uint8_t kind{};
  bool born{}, activated{};
  double event_time{};
...
public:
  ...
  std::uint32_t add_cognits(std::uint32_t count, double activity = 0, double threshold = .25, double confidence = .5) {
    auto first = (std::uint32_t)graph_.cognit_count();
    for (std::uint32_t i = 0; i < count; ++i) graph_.add_cognit(activity, threshold, confidence);
    resize_scratch();
    return first;
  }
  void set_activity(std::uint32_t id, double value) {
    catch_up_homeostasis(id);
    if (!cognit_alive(id))
      throw std::out_of_range("dead or invalid Cognit");
    graph_.activity.at(id) = value;
    ++state_revision_;
  }
  void set_refractory(std::uint32_t id, std::uint16_t value) {
    catch_up_homeostasis(id);
    if (!cognit_alive(id))
      throw std::out_of_range("dead or invalid Cognit");
    graph_.refractory.at(id) = value;
    ++state_revision_;
  }
  ...
  void update_relation(RelationHandle h, const PersistedRelation &r);
  std::vector<std::uint32_t> outgoing_targets(std::span<const std::uint32_t> sources) const;
  bool remove_relation(RelationHandle handle) { return graph_.relations.erase(handle); }
  bool relation_handle_valid(RelationHandle handle) const { return graph_.relations.valid(handle); }
  static constexpr std::size_t provisional_relation_bytes() { return RelationStore::provisional_record_bytes(); }
  static constexpr std::size_t consolidated_relation_bytes() { return RelationStore::consolidated_record_bytes(); }
  ...
  std::size_t cognit_count() const { return graph_.cognit_count(); }
  std::size_t relation_count() const { return graph_.relation_count(); }
  std::size_t reserved_bytes() const;
  std::size_t provisional_count() const { return graph_.relations.provisional_count(); }
  std::size_t consolidated_count() const { return graph_.relations.consolidated_count(); }
...
private:
  ...
  struct ActionSourceKey {
    std::uint32_t s;
    std::uint8_t a;
    bool operator==(const ActionSourceKey &) const = default;
  };
  struct ASHash {
    std::size_t operator()(const ActionSourceKey &x) const { return (std::size_t(x.s) << 8) ^ x.a; }
  };
  ...
  std::vector<Experience> evidence_slots_;
  std::unordered_map<std::uint32_t, std::uint32_t> source_counts_, target_counts_;
  std::unordered_map<ActionSourceKey, std::uint32_t, ASHash> action_source_counts_;
  std::unordered_map<std::uint32_t, SlotMask> before_slots_, after_slots_;
  std::unordered_map<ActionSourceKey, SlotMask, ASHash> before_action_slots_;
  std::vector<double> incoming_, absence_, frontier_energy_, next_energy_;
  std::vector<std::uint32_t> incoming_gen_, absence_gen_, visited_gen_, frontier_ids_, next_ids_, touched_, candidate_targets_;
  ...
  void evict_slot(std::size_t);
  std::size_t evidence_reserved_bytes() const;
  std::uint32_t support(std::uint32_t, std::uint32_t) const;
  std::uint32_t action_support(std::uint32_t, std::uint8_t, std::uint32_t) const;
  void discover_targets(const SlotMask &, std::vector<std::uint32_t> &) const;
  static bool mask_empty(const SlotMask &);
  std::vector<std::uint64_t> relation_confidence_cache_tick_;
  std::vector<double> relation_confidence_cache_base_, relation_confidence_cache_decay_, relation_confidence_cache_value_;
};
} // namespace se
