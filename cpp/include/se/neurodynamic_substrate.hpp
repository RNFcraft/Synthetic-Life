...
struct AssemblyRecord {
  ...
  std::vector<std::array<std::uint32_t, 2>> temporal_edges;
  std::vector<std::array<double, 2>> member_support;
  std::vector<std::array<double, 3>> temporal_support;
  std::uint32_t support{};
  double decayed_support{}, first_seen{}, last_seen{};
  bool candidate{}, consolidated{};
};
struct AssemblyMatch {
  std::uint64_t assembly_id{};
  double confidence{}, completeness{}, temporal_match{}, time{};
  std::vector<std::uint32_t> matched_members;
};
...
struct NeuroSnapshot {
  ...
  double now{};
  std::uint64_t next_sequence{}, event_guard{};
  ...
  std::vector<std::uint32_t> source, target;
  std::vector<double> weight, delay;
  std::vector<std::uint8_t> polarity, plasticity_enabled;
  bool assembly_tracking_enabled{};
  double assembly_window{}, assembly_decay_tau{}, assembly_member_ratio_threshold{}, assembly_candidate_similarity_threshold{}, assembly_temporal_similarity_threshold{};
  std::uint32_t assembly_min_support{}, assembly_min_members{}, assembly_max_members{}, assembly_consolidation_support{}, max_assembly_candidates{}, max_consolidated_assemblies{}, max_temporal_edges{}, max_recent_spikes{};
  std::uint64_t next_assembly_id{};
  std::vector<NeuralEvent> recent_spikes;
  std::vector<AssemblyRecord> assemblies;
  std::vector<AssemblyMatch> recent_matches;
...
public:
  ...
  std::size_t micro_kappa_count() const { return potential_.size(); }
  std::size_t micro_rho_count() const { return source_.size(); }
...
private:
  struct Compare {
    bool operator()(const NeuralEvent &a, const NeuralEvent &b) const { return a.time > b.time || (a.time == b.time && a.sequence > b.sequence); }
  };
  ...
  bool assembly_tracking_enabled_{};
  double assembly_window_{}, assembly_decay_tau_{}, assembly_member_ratio_threshold_{}, assembly_candidate_similarity_threshold_{}, assembly_temporal_similarity_threshold_{};
  std::uint32_t assembly_min_support_{}, assembly_min_members_{}, assembly_max_members_{}, assembly_consolidation_support_{}, max_assembly_candidates_{}, max_consolidated_assemblies_{}, max_temporal_edges_{}, max_recent_spikes_{};
  std::uint64_t next_assembly_id_{};
  ...
  std::vector<std::uint32_t> source_, target_;
  std::vector<double> weight_, delay_;
  std::vector<std::uint8_t> polarity_, plasticity_enabled_;
  std::vector<std::vector<std::uint32_t>> outgoing_, incoming_;
  std::priority_queue<NeuralEvent, std::vector<NeuralEvent>, Compare> queue_;
  NeuroTelemetry telemetry_{};
  std::vector<NeuralEvent> recent_spikes_;
  std::vector<AssemblyRecord> assemblies_;
  std::vector<AssemblyMatch> recent_matches_;
  ...
  void schedule(std::uint32_t target, double amplitude, double time);
  void deliver(double time, const std::vector<NeuralEvent> &events);
  double pre_trace_at(std::uint32_t id, double time) const;
  double post_trace_at(std::uint32_t id, double time) const;
  double homeostatic_bias_at(std::uint32_t id, double time) const;
  ...
  void emit_bridge_event(std::uint64_t assembly_id, double time, double confidence, AssemblyBridgeEventKind kind);
};
} // namespace se
