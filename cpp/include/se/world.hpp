...
class World {
public:
  World(int width=30,int height=30,int radius=4):width_(width),height_(height),radius_(radius),snapshot_channel_(std::make_shared<RenderSnapshotChannel>()){publish_snapshot();}
  World(const World& other); World& operator=(const World& other);
